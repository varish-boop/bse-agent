...
def get_live_stock_data(stock_name):
    try:
        ...
        # Use last available closing price (works during and after market hours)
        ...
        # MACD calculation directly from yfinance
        ...
        # Market cap and PE from info
        market_cap = info.get("marketCap", 0)
        market_cap_str = f"₹{market_cap/10000000:.0f} Cr" if market_cap else "N/A"
        ...
        return {
            ...
            "week52_high": f"₹{week52_high:.2f}",
            "week52_low": f"₹{week52_low:.2f}",
            "rsi": f"{rsi_val:.1f}",
...
def analyze_with_gemini(stock_name, live_data):
    ...
    # If Gemini completely fails use rule based fallback from real yfinance data
    rsi_val = float(live_data.get("rsi", 50))
    macd = live_data.get("macd", "Neutral")
    price_str = live_data.get("price", "₹0").replace("₹", "").replace(",", "")

    try:
        price = float(price_str)
        target = f"₹{price * 1.10:.2f}"
...
━━━━━━━━━━━━━━━
{change_emoji} Price: *{live_data.get('price')}* ({live_data.get('change')})
...
def run_after_market():
    ...
    for g in gainers[:5]:
        report += f"🟢 *{g['stock']}* {g['price']} ({g['change']})\n"
    report += "\n📉 *TOP LOSERS TODAY*\n"
    for l in losers[:5]:
        report += f"🔴 *{l['stock']}* {l['price']} ({l['change']})\n"
...
print("BSE Final Agent started. All N/A issues fixed.")
print("MACD, RSI, Support, Resistance all calculated from real yfinance data.")

send_telegram("""🚀 *BSE AI AGENT - FINAL VERSION*

All N/A issues fixed.

✅ Real BSE prices via yfinance
✅ RSI calculated from real data
✅ MACD calculated from real data
✅ Support and Resistance from real data
✅ Target and Stop Loss calculated automatically
✅ Retry logic for Gemini failures
✅ IST timezone fixed

Type any stock name to test right now.
